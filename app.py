...
def cb_pdf_upload(files, state):
    ...
    if not files:
        return "❌ No files received.", state

    try:
        file_paths = [f.name if hasattr(f, "name") else f for f in files]
        documents = load_documents(file_paths)
        if not documents:
            return "❌ No readable text found in uploaded files.", state
        ...
        return f"✅ Loaded {len(files)} file(s). PDF text ready for chat.", state

    except Exception as e:
        return f"❌ PDF upload error: {e}", state
...
def cb_pdf_chat(message, history, state):
    ...
    if not vs_path or not os.path.exists(vs_path):
        history.append((message, "⚠️ Document vectorstore not found. Re-upload the PDF."))
        return history, "", state
    ...
    # 🔹 EXPLICIT: convert query → embedding
    query_embedding = embeddings.embed_query(message)

    # 🔹 Vector similarity search (top-4)
...
>>>>>>> 340e60dc75f3073dca4463d65b333c4c0580935b
User profile:
- Age: {profile.get("age")}
...
def cb_create_profile(age, height, weight, fitness_level, gender, health_issues, state):
    ...
    return f"✅ Profile created and saved.\n📁 {path}\nNow choose a mode below.", state
...
def cb_analyze(img, state):
    ...
    profile = state.get("user_profile")
    if not profile:
        return "⚠️ Create profile first.", state
...
def cb_image_chat(message, history, state):
    ...
    if not image_state or image_state.get("error"):
        history.append((message, "⚠️ Analyze an image first."))
        return history, "", state

    vs_path = image_state.get("profile_vectorstore_path")
    if not vs_path or not os.path.exists(vs_path):
        history.append((message, "⚠️ Image vectorstore not found. Re-run image analysis."))
        return history, "", state
