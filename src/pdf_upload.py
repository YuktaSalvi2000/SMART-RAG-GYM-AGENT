...
def upload_and_index(files, state, save_dir="stores/pdf"):
    ...
    try:
        ...
        print("🗂 Files received:", file_paths)

        documents = load_documents(file_paths)
        if not documents:
            return "❌ No readable text found in the uploaded files.", state
        ...
        state["pdf_raw_preview"] = pdf_raw_preview  # ✅ added (for prompt-based chat)
        ...
        return f"✅ Uploaded {len(files)} file(s); created {len(chunks)} chunks.", state
