...
def analyze_image(
    ...
    if not user_profile:
        return {"error": "⚠️ User profile is required for body analysis."}

    if img is None:
        return {"error": "⚠️ Please upload an image."}
...
"""
        ...
        if not image_analysis_texts:
            return {"error": "⚠️ Could not analyze the image(s). Please try again with a clearer image."}
        ...
        if document_vectorstore:
            ...
            analysis_text += f"\n\n🔍 Based on your uploaded workout/diet, here's a feedback match:\n{feedback}"
